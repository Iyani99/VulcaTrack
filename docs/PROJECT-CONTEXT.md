...
| Test harness | `vulcatrack/tests/` — dependency-free CLI runner, no Composer/PHPUnit. `php vulcatrack/tests/run.php` → **82 tests / 429 assertions** (unit + integration + end-to-end HTTP). MariaDB must be running for the integration/http suites; geocoding HTTP tests use a fake provider (`VULCATRACK_GEOCODER_FAKE`), never the network. |
