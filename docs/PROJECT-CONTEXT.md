...
| **Current project phase** | **Phases 1–5 + Phase 4.5 COMPLETE. Phase 5 (POS & inventory) closed 2026-09-23** after an end-to-end hardening pass: the minimal Admin app shell, the full Inventory module, `Money` (integer centavos), the **Sales foundation** (`SaleRepository` + atomic `SaleService`, Decisions 61–63), the **POS** (`admin/pos.php` — session cart, optional customer link / walk-in, cash tender/change, checkout through `SaleService`) and the **printable Transaction Summary** (`admin/transaction-summary.php`, Decision 50). **Phase 6 (admin OTG handling + reporting) has NOT started.** Phase 5 pre-decisions are Decisions 49–57; the sales-foundation decisions are 61–63. |
| **Last updated** | 2026-09-23 (rev. 13 — end-of-Phase-5 hardening + documentation pass; Phase 5 COMPLETE; test count 156/1396) |
...
| MySQL/MariaDB | Starts; user `root`, **no password** (XAMPP default). This install's **global `sql_mode` is non-strict**; VulcaTrack forces `STRICT_TRANS_TABLES` on its own PDO sessions in `includes/db.php` (Decision 64) — the global / `my.ini` is not changed. |
...
| Application code | At `C:\IPT102\vulcatrack\`. **Phases 3–4:** auth (customer + admin, hardened sessions, guards, CLI admin seeding) **and** the full customer side — dashboard, profile, saved vehicles (soft-delete), OTG rescue submission (browser geolocation **or landmark/address search**, draggable-marker confirmation → frozen ETA; geocode proxy at `customer/geocode.php`), request history + customer-facing status. **Phase 5 (complete):** Admin app shell (`src/Views/partials/admin_*.php`, nav Dashboard/POS/Inventory), `Money` helper (integer centavos), `ItemRepository` + the Inventory module (`admin/inventory.php` browse + `admin/item-edit.php` create/edit + activate/deactivate), **and the Sales foundation** — `src/Repository/SaleRepository.php` + `src/Service/SaleService.php` (+ `SaleException`): one class owns the checkout transaction, DB-authoritative pricing, frozen `unit_price`, integer-centavo totals, `SELECT … FOR UPDATE` stock locking, product-only deduction, full rollback (Decisions 61–63) — **and the POS UI** `admin/pos.php` (session cart `src/Service/PosCart.php`, optional existing-customer link via `CustomerRepository::search()`, server-checked cash tender/change, checkout through `SaleService`). **Printable Transaction Summary** `admin/transaction-summary.php?id=<sale_id>` (Decision 50 non-official sale document, linked from the POS success card). Not built yet: admin OTG handling, Tireman management, reports (Phase 6). |
| Test harness | `vulcatrack/tests/` — dependency-free CLI runner, no Composer/PHPUnit. `php vulcatrack/tests/run.php` → **156 tests / 1396 assertions / 25 files** (unit + integration + end-to-end HTTP). MariaDB must be running for the integration/http suites (XAMPP starts it manually); geocoding HTTP tests use a fake provider (`VULCATRACK_GEOCODER_FAKE`), never the network. |
...
| `docs/decisions/project-decisions.md` | **Authoritative** confirmed decisions, scope, change-control. Decisions numbered 1–64 + open questions + conflict log + revision history. |
...
- Transaction completion; printable **Transaction Summary** as an HTML/print view — a non-official transaction reference, not an official BIR invoice (**no receipt table** — see §10).
...
- POS is **admin-operated, in-person**. No online checkout. Reachable through a minimal admin shell/nav (Dashboard · POS · Inventory) — approved as a Phase 5 prerequisite (Decision 53). **Sales reporting / history UI is Phase 6, not Phase 5** (Decision 49); `SaleRepository` exposes receipt-read methods (`findSaleForReceipt` / `listSaleItems`) but no reporting page is built.
...
- **Server-side foundation built (Decisions 61–63):** `SaleService::checkout()` owns the one checkout transaction; the caller passes only item ids, quantities and an optional existing `customer_id`. The DB is authoritative for price (frozen into `sale_items.unit_price`), subtotals and `sales.total_amount` (integer-centavo `Money`), and `sale_date`. Stock is locked with `SELECT … FOR UPDATE` and deducted with a guarded, non-negative, product-only update; any failure rolls the whole sale back. The POS UI is a thin caller of this.
- **POS UI built (2026-09-23):** `admin/pos.php` (admin-only; every cart change POST + CSRF + PRG with a one-time session flash). Cart = `PosCart` in `$_SESSION['pos_cart']` holding only `[item_id => quantity]` + optional `customer_id` — never prices; display re-reads the DB. Limits (POS-layer, far below the column ceilings): **max 50 distinct items, max 9,999 of one item** (`PosCart::MAX_LINES` / `MAX_QUANTITY`). Adding re-uses one line per item; active items only; friendly product-stock pre-check (authoritative check stays in `SaleService`). Customer link = search existing customers by name/email/contact, blank = walk-in, never creates accounts. Checkout posts the displayed total as `expected_total_centavos`; `SaleService` compares it with the authoritative total **inside its transaction** and rolls back on mismatch (stale price / cart changed in another window) — it is an assertion, never a value. Submitted quantities must equal the session cart (unsaved edits never slip into a sale). Cash tender parsed with `Money`, must be ≥ the total; change shown once after the sale, never stored. Failed checkout re-renders with the cart and typed cash intact; success clears the cart. The success card links to the Transaction Summary for the new sale.
...
- **Application-layer validation for Phase 5 (Decision 56):** `price >= 0`, `quantity > 0`, `stock >= 0`, enforced in the service/repository layer. The approved Phase 2 schema is **not** changed to add CHECK constraints unless a real correctness problem forces a rethink.
- **Session-backed cart (Decision 57):** a temporary `$_SESSION` cart is acceptable *only* to preserve cart contents across a validation/stock error. It is not a persistent e-commerce cart.
- **Receipt (Decision 50):** printable **HTML/print view** from `sales` + `sale_items`. Fields: shop name + address, `sale_id` as the receipt number, sale date/time, cashier/Admin name, linked customer name or "Walk-in", per line {name, quantity, frozen unit price, subtotal}, total, print button/styling. **No receipt table**, no TIN/BIR/tax fields, no "official receipt" claim. **Non-official transaction reference (Decision 50, clarified 2026-09-23):** "receipt" is only the working term — the printed document gets a neutral title (e.g. "Sales Transaction Slip" / "Transaction Summary") and a note such as *"For transaction reference only. Not an official BIR invoice."*; it is not an Official Receipt / BIR-registered invoice and does not replace the shop's legally required invoicing. Official invoicing (TIN/VAT/BIR accreditation) is outside the approved scope. **Built 2026-09-23:** `admin/transaction-summary.php?id=<sale_id>` — admin-only GET; strict id check (anything but 1..2147483647 or an unknown sale → 404 in the shell); renders only recorded values via `SaleRepository::findSaleForReceipt()` + `listSaleItems()` (quantity, frozen unit price, subtotal, total, sale_date — never `items.price`, no recalculation); `Money::format` for display. **Schema limitation (not changed):** item / cashier / customer *names* are joined live by id (no name snapshot columns), so a later rename shows the current name; all money and dates are frozen. Print: Print button (`window.print()`), `@media print` scoped by `body.is-printdoc` (optional `$bodyClass` in `admin_top.php`) hides the app bar / footer / `.no-print` controls.
...
### 16.4 Phase 5 known limitations / deferred items (recorded at phase close, 2026-09-23)

None of these is a correctness, security or data-integrity blocker. Money, dates and stock on recorded sales are always the frozen/authoritative values.
- **Transaction Summary names are live, not frozen.** Item / cashier / customer *names* are joined by id at display time (the approved schema has no name snapshot columns), so a later rename shows the new name on old sales. Prices, quantities, subtotals, total and `sale_date` are frozen. Changing this would be a schema decision.
- **No sale lookup / history screen.** A Transaction Summary is reached from the POS success card (or `?id=` directly). Sales history / reporting is Phase 6 (Decision 49).
- **Inventory `?saved=` notice** reappears on refresh (cosmetic). The POS already uses a one-time session flash; Inventory could adopt it in Phase 7.
- **POS catalog on phones:** the item list keeps the standard `.table-scroll` strip, so the *Add* column is reached by swiping the strip (verified reachable, 58×40 px target). The cart total is repeated beside the cash box so it is always visible.
- **Date/time display** is the raw `Y-m-d H:i:s` string, as on the other pages; friendlier formatting is Phase 7 polish.
- Carried over (not Phase 5 work): `session.cookie_secure` must be enabled once served over HTTPS; `config.example.php` Nominatim `referer` still names localhost; the repo-root `README.md` truncation (§16.3).
...
| **Phase 5** | POS & inventory — unified `items`, one inventory module, POS with walk-in support and stock deduction, printable HTML receipt, minimal admin shell. Reporting/history UI is **not** in Phase 5 (Decision 49). *(**COMPLETE 2026-09-23.** Started 2026-09-07: admin shell + `Money` helper + `ItemRepository` + the full Inventory module; the Sales foundation — `SaleRepository` + atomic `SaleService` (Decisions 61–63) — 2026-09-08; the POS (session cart, customer link, cash tender/change, checkout) and the printable Transaction Summary 2026-09-23; closed after an end-to-end walkthrough through Apache, a phone/LAN check and a documentation pass. Known limitations / deferred items: §16.4.)* |
...
| Date | Change |
|---|---|
| 2026-09-23 (rev. 13) | **End-of-Phase-5 hardening + documentation pass — Phase 5 COMPLETE. No schema change.** End-to-end walkthrough in real Chrome through Apache :80 (customer self-registration → CLI-provisioned admin → create product + service → Inventory → POS add / merge / update / remove / walk-in / customer link / insufficient cash refused / checkout → DB verified (one sale, frozen prices, product stock 15→11, service stock NULL) → Transaction Summary → print preview/PDF → Inventory). Phone/LAN check via the LAN IP at 360/390/430/768/1280 px: no page overflow, no overlaps, no JS errors, every request stayed on the LAN host. Fixes: POS shows **Total due** beside the cash box (the cart table’s total sat behind the scroll strip on phones); Inventory activate/deactivate now **returns to the same filtered list**; stale “future POS endpoint” comment in `SaleService`. Docs: this file (header, §0, §3, §6, §16.4 new, §18), decision-record status, app / tests / root READMEs. Tests: `AdminPosTest` + `AdminInventoryMutationTest` extended — **156 passed / 1396 assertions / 25 files**. |
| 2026-09-23 (rev. 12) | **Phase 5 chunk — printable Transaction Summary. No schema change.** New `admin/transaction-summary.php` (Decision 50 non-official sale document: shop name/address from `config/shop.php`, sale no., sale date/time, cashier, customer or Walk-in, lines with qty / frozen unit price / subtotal, total, note *"For transaction reference only. Not an official BIR invoice."*, Print button + print CSS). Reuses the existing `SaleRepository` read methods unchanged. POS success card links to it; dashboard wording updated. `admin_top.php` gained an optional `$bodyClass`. Noted limitation: display names are live joins (schema has no name snapshot). Updated the header, §0, §3, §10, §18. Tests: +`http/AdminTransactionSummaryTest`, `AdminPosTest` extended — **156 passed / 1386 assertions / 25 files**. **Still exactly 8 tables.** |
| 2026-09-23 (rev. 11) | **Documentation / scope alignment only — no code, no schema change.** Owner approved the `SaleService` `expected_total_centavos` behaviour; the decision record now clarifies it under **Decision 62** (optional stale-state assertion, never an authoritative price/total; mismatch → full rollback). **Decision 50** clarified: the printable sale document is a non-official transaction reference (neutral title + "not an official BIR invoice" note; not a replacement for the shop's legally required invoicing). Updated §10 (receipt bullet). |
| 2026-09-23 (rev. 10) | **Phase 5 chunk — POS UI + session cart + checkout integration. No schema change.** Replaced the `admin/pos.php` placeholder with the working POS (see §10). Added `src/Service/PosCart.php` + `PosCartException.php` (session cart, Decision 57; new POS-layer limits 50 distinct items / 9,999 per item) and `CustomerRepository::search()` (optional existing-customer link, Decision 51). **`SaleService::checkout()` gained an optional `expected_total_centavos` assertion** — checked against the authoritative total inside the transaction; any mismatch is a `SaleException` + full rollback, so a stale screen can never record a different amount than the cashier quoted (the caller still cannot choose any price/total). Cash tender/change validated in integer centavos in the page, never persisted (Decision 54). Fixed two mislabelled decision numbers in §10 (validation = 56, session cart = 57). Updated the header, §0, §3, §4, §10, §18. Tests: +`unit/PosCartTest`, +`http/AdminPosTest`, +3 `SaleServiceTest` cases, +1 `RepositoryTest` case, `AdminShellTest` updated — **155 passed / 1214 assertions / 24 files**. Receipt not built. **Still exactly 8 tables.** |
| 2026-09-08 (rev. 9) | **Phase 5 chunk — Sales foundation. No schema change.** Added `src/Repository/SaleRepository.php`, `src/Service/SaleService.php`, `src/Service/SaleException.php`; `ItemRepository` gained `lockForUpdate()` + `decrementStock()` (used only by `SaleService`), and `ItemRepository` / `SaleRepository` are no longer `final` so the mock-library-free test harness can subclass them for failure injection. **Decisions 61–63:** `SaleService` is the single owner of the checkout transaction (begin/commit/rollback, no nesting, full rollback on any failure); the server is authoritative for price (frozen into `sale_items.unit_price`), subtotals, `sales.total_amount` (integer-centavo `Money`) and `sale_date`; overselling is prevented with `SELECT … FOR UPDATE` + a guarded non-negative product-only decrement (services never touch stock). A same-day pre-commit integrity audit added four small hardening fixes (no redesign): `sale_items.quantity` INT-ceiling enforced per-line + on merged totals; overflow-safe money bounds (`intdiv` before multiply, subtraction before add); `AdminRepository::findById()` so a stale recording-admin is a clean `SaleException` not a raw FK error; rollback wrapped so it can't mask the original exception. **Decision 64 (follow-on):** `includes/db.php` now adds `STRICT_TRANS_TABLES` to every VulcaTrack PDO session — session-scoped only, via an `IF(FIND_IN_SET…)` / `CONCAT_WS` / `NULLIF` expression that is correct whether the inherited mode already has strict, lacks it, or is empty; no `my.ini`/global change — so an over-long string / out-of-range number is rejected, not silently truncated/clamped; verified compatible with every completed module (incl. OTG DECIMAL(10,7) coords — fractional rounding is a warning, not an error). Updated the header, §0, §3, §10, §18. Tests: +3 files (`SaleRepositoryTest`, `SaleServiceTest`, `DbSessionTest`), **143 passed / 1028 assertions / 22 files**. **Still exactly 8 tables. POS UI not started.** |
