...
| **Current project phase** | **Phases 1–4 + Phase 4.5 COMPLETE. Phase 5 (POS & inventory) IN PROGRESS (started 2026-09-07):** the minimal Admin app shell, the full Inventory module, the **Sales foundation** (`SaleRepository` + atomic `SaleService`, Decisions 61–63), the **POS UI** (`admin/pos.php` — session cart, optional customer link, cash tender/change, checkout through `SaleService`) and the **printable Transaction Summary** (`admin/transaction-summary.php`, Decision 50) are done and tested; **a final Phase 5 hardening/doc pass is what remains.** Phase 5 pre-decisions are Decisions 49–57; the sales-foundation decisions are 61–63. |
| **Last updated** | 2026-09-23 (rev. 12 — printable Transaction Summary; test count 156/1386. Rev. 11 same day: Decisions 50/62 clarified. Rev. 10 same day: POS UI + session cart + checkout integration) |
...
| Application code | At `C:\IPT102\vulcatrack\`. **Phases 3–4:** auth (customer + admin, hardened sessions, guards, CLI admin seeding) **and** the full customer side — dashboard, profile, saved vehicles (soft-delete), OTG rescue submission (browser geolocation **or landmark/address search**, draggable-marker confirmation → frozen ETA; geocode proxy at `customer/geocode.php`), request history + customer-facing status. **Phase 5 so far:** Admin app shell (`src/Views/partials/admin_*.php`, nav Dashboard/POS/Inventory), `Money` helper (integer centavos), `ItemRepository` + the Inventory module (`admin/inventory.php` browse + `admin/item-edit.php` create/edit + activate/deactivate), **and the Sales foundation** — `src/Repository/SaleRepository.php` + `src/Service/SaleService.php` (+ `SaleException`): one class owns the checkout transaction, DB-authoritative pricing, frozen `unit_price`, integer-centavo totals, `SELECT … FOR UPDATE` stock locking, product-only deduction, full rollback (Decisions 61–63) — **and the POS UI** `admin/pos.php` (session cart `src/Service/PosCart.php`, optional existing-customer link via `CustomerRepository::search()`, server-checked cash tender/change, checkout through `SaleService`). **Printable Transaction Summary** `admin/transaction-summary.php?id=<sale_id>` (Decision 50 non-official sale document, linked from the POS success card). Not built yet: admin OTG handling, reports (rest of Phase 5 + Phase 6). |
| Test harness | `vulcatrack/tests/` — dependency-free CLI runner, no Composer/PHPUnit. `php vulcatrack/tests/run.php` → **156 tests / 1386 assertions / 25 files** (unit + integration + end-to-end HTTP). MariaDB must be running for the integration/http suites (XAMPP starts it manually); geocoding HTTP tests use a fake provider (`VULCATRACK_GEOCODER_FAKE`), never the network. |
...
| **Phase 5** | POS & inventory — unified `items`, one inventory module, POS with walk-in support and stock deduction, printable HTML receipt, minimal admin shell. Reporting/history UI is **not** in Phase 5 (Decision 49). *(IN PROGRESS since 2026-09-07: admin shell + `Money` helper + `ItemRepository` + the full Inventory module done and tested; the Sales foundation — `SaleRepository` + atomic `SaleService` (Decisions 61–63) — done and tested 2026-09-08; the POS UI (session cart, customer link, cash tender/change, checkout) done and tested 2026-09-23; the printable Transaction Summary done and tested 2026-09-23; a final Phase 5 hardening/doc pass remains.)* |
